# Dark Box Test
## WaveDumpConfig_darkbox.txt
This is the config file for Wavedump in the darkbox test. We set the ``EXTERNAL_TRIGGER`` to be ``ACQUISITION_ONLY``, which means that the digitizer only takes data when receiving an external trigger signal, in our case the square wave from the function generator. The meaning of other entries is in the ``WaveDump_config.docx``.
