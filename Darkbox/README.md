...
This is the config file for Wavedump in the darkbox test. We set the ``EXTERNAL_TRIGGER`` to be ``ACQUISITION ONLY``, which means that the digitizer only takes data when receiving an external trigger signal, in our case is the square wave from the function generator.
